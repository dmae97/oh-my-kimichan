...
export function discoverRoutingInventory(projectRoot = getRoutingProjectRoot()): RoutingInventory {
  ...
  for (const file of mcpConfigFiles(root, mcpScope)) {
    for (const server of readMcpServerNames(file.path)) {
      if (!mcpServers.has(server)) mcpServers.set(server, file.source);
    }
  }
...
function mcpConfigFiles(root: string, scope: RoutingInventory["mcpScope"]): Array<{ path: string; source: RouteSource }> {
  if (scope === "none") return [];
  const files: Array<{ path: string; source: RouteSource }> = [
    { path: join(root, ".omk", "mcp.json"), source: "project" },
    { path: join(root, ".kimi", "mcp.json"), source: "project" },
  ];
  if (scope === "all") {
    files.push({ path: join(homedir(), ".kimi", "mcp.json"), source: "global" });
  }
  return files;
}
